...
disallowed_text = ["abc", "def"]  # List of disallowed text/strings
disallowed_chars = re.compile("[a-zA-Z]")

valid_scientific_buttons = ["sin", "cos", "tan", "log", "sqrt", "pi", "e", "^"]
...
def add_to_calculation(symbol):
    ...
    # If the symbol is a valid scientific function, do not block it
    if any(symbol.startswith(func) for func in valid_scientific_buttons):
        calculation += str(symbol)
        entry.delete(0, tk.END)
        entry.insert(0, calculation)
        return

    # Check if the symbol is in the disallowed text list
    if any(text in symbol for text in disallowed_text):
        entry.delete(0, tk.END)
        entry.insert(0, "Text not allowed")
        return

    # Check for disallowed alphabetic characters
    if disallowed_chars.search(symbol):
        entry.delete(0, tk.END)
        entry.insert(0, "Text not allowed")
        return
    
    # If the input is valid, add it to the calculation
    calculation += str(symbol)
    entry.delete(0, tk.END)
    entry.insert(0, calculation)
...
def evaluate_calculation():
    global calculation
    try:
        # Evaluate the calculation and allow only math functions (like sqrt, sin, etc.)
